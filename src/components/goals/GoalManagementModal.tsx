      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-transparent">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-emerald-500/15 dark:bg-emerald-500/25 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300">
                  Cashew Smart Targets
                </span>
              </div>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                Track progress towards dreams, emergency funds & large purchases
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 bg-slate-50/70 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Total Target Pool</span>
            <div className="text-xl font-bold text-slate-900 dark:text-white">{formatINR(totalTarget)}</div>
            <span className="text-[11px] text-slate-400 mt-1 block">{activeGoals.length} configured targets</span>
          </div>

          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
            <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 block mb-1">Total Saved & Allocated</span>
            <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{formatINR(totalSaved)}</div>
            <span className="text-[11px] text-slate-400 mt-1 block">{formatINR(Math.max(0, totalTarget - totalSaved))} remaining to hit all goals</span>
          </div>

          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Total Completion</span>
              <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">{overallProgress}%</span>
            </div>
            <div className="w-full bg-slate-100 dark:bg-slate-700 h-2.5 rounded-full overflow-hidden mt-2">
              <div
            ...
            <span className="text-[11px] text-slate-400 mt-2 block">
              {activeGoals.filter(g => g.status === 'COMPLETED').length} goals completed
            </span>
          </div>
        </div>
            ...
            <div className="bg-emerald-500/5 dark:bg-emerald-950/20 border border-emerald-500/20 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
                  Quick Start Templates (Cashew Best Practices)
                </span>
              </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 flex items-center justify-between text-xs text-slate-500">
          <span>Goals are stored locally and synced across your financial balance ledger.</span>
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 rounded-xl font-medium transition-colors"
          >
            Close
          </button>
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
              {(!historyGoal.allocations || historyGoal.allocations.length === 0) ? (
                ...
                historyGoal.allocations.map(alloc => (
                  <div
                    key={alloc.id}
